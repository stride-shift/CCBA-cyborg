-- Enhance cohorts table with all missing columns and fix user_profiles relationship (SAFE VERSION)
...
-- Step 2: Add missing columns to cohorts table (safe version)
...
UPDATE cohorts SET 
    ...
    organization_name = COALESCE(organization_name, 'Cyborg Habit Co.'),
    cohort_type = COALESCE(cohort_type, 'standard'),
    enrollment_start_date = COALESCE(enrollment_start_date, start_date - INTERVAL '2 weeks'),
    enrollment_end_date = COALESCE(enrollment_end_date, start_date - INTERVAL '1 day'),
    ...
    meeting_schedule = COALESCE(meeting_schedule, 'Weekly check-ins and daily challenges'),
    completion_criteria = COALESCE(completion_criteria, 'Complete all weekly challenges and reflections'),
    tags = COALESCE(tags, ARRAY['habit-formation', 'personal-development'])

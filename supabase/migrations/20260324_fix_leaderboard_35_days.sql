-- Fix leaderboard and admin stats for 35-day (and growing) programme
-- - Count ALL challenge days (not just 1-15)
-- - Include pre-survey (day 0), exclude post-survey
-- - Journey % uses dynamic challenge count as denominator
...
-- Determine if cohort uses customized challenges
cohort_type AS (
    SELECT EXISTS(
...
-- Count total available challenges dynamically
...
-- Total possible = challenge days + 1 (pre-survey only, no post-survey)
max_days AS (
...
-- Get all day completions (normal + customized)
...
-- Count ALL valid days (not just 1-15)
...
-- Only pre-survey counts (no post-survey)
survey_days AS (
...
BEGIN
    ...
    -- Day completions: ALL days, not just 1-15
    program_completions AS (
    ...
    -- Pre-survey only, no post-survey
    survey_days AS (

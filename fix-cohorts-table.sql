-- Fix Cohorts Table - Handle existing table structure
...
-- WARNING: This will delete any existing cohort data
DROP TABLE IF EXISTS cohorts CASCADE;

-- Step 3: Create the complete cohorts table
...
CREATE INDEX idx_cohorts_organization ON cohorts(organization_name);
...
CREATE INDEX idx_cohorts_enrollment ON cohorts(enrollment_start_date, enrollment_end_date);
...
-- Step 8: Create a view for cohort statistics
...
-- Step 9: Create function to auto-update cohort status
...
-- Step 12: Show table structure to confirm
SELECT column_name, data_type, is_nullable, column_default
FROM information_schema.columns 
WHERE table_name = 'cohorts' 
ORDER BY ordinal_position; 
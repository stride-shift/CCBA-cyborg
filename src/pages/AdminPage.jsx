      ...
      <div className="container mx-auto px-4 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            ...
            <div className="glassmorphism rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-white">Challenge Progress Overview</h2>
                <div className="flex gap-2">
                  <button onClick={() => setChartType('line')} className={`px-3 py-1 rounded-full text-sm font-semibold ${chartType === 'line' ? 'glassmorphism text-white' : 'bg-white/20 text-white hover:bg-white/30 border border-white/30'}`}>Line</button>
                  <button onClick={() => setChartType('bar')} className={`px-3 py-1 rounded-full text-sm font-semibold ${chartType === 'bar' ? 'glassmorphism text-white' : 'bg-white/20 text-white hover:bg-white/30 border border-white/30'}`}>Bar</button>
                  <button onClick={() => setChartType('pie')} className={`px-3 py-1 rounded-full text-sm font-semibold ${chartType === 'pie' ? 'glassmorphism text-white' : 'bg-white/20 text-white hover:bg-white/30 border border-white/30'}`}>Pie</button>
                </div>
              </div>
              {/* Mock chart area */}
                ...
                {chartType === 'bar' && (
                  <div className="w-2/3 h-48 bg-white/20 rounded-lg flex items-end backdrop-blur-sm border border-white/30">
                    <div className="w-1/2 h-40 bg-gradient-to-t from-blue-400/80 to-blue-300/60 mx-2 rounded-t-lg flex items-end justify-center">
                      <span className="text-xs text-white mb-2 font-semibold">Not Started</span>
                    </div>
                    <div className="w-1/2 h-8 bg-gradient-to-t from-green-400/80 to-green-300/60 mx-2 rounded-t-lg flex items-end justify-center">
                      <span className="text-xs text-white mb-2 font-semibold">Completed</span>
                    </div>
                  </div>
                )}
                {chartType === 'line' && (
                  <div className="w-full h-48 flex items-center justify-center text-white glassmorphism rounded-lg">[Line Chart Placeholder]</div>
                )}
                {chartType === 'pie' && (
                  <div className="w-32 h-32 rounded-full bg-gradient-to-br from-blue-400/60 to-green-400/60 flex items-center justify-center text-white font-semibold">[Pie Chart]</div>
                )}
              </div>
            </div>
            {/* Challenge Completion Stats */}
            <div className="glassmorphism rounded-2xl p-6">
              <h2 className="text-xl font-bold text-white mb-4">Challenge Completion Stats</h2>
              <div className="mb-2 flex justify-between text-white"><span>User Engagement Rate</span><span>0.0%</span></div>
              ...
              <div className="mb-2 flex justify-between text-white"><span>Platform Challenge Completion</span><span>0.0%</span></div>
              ...
              <div className="mb-2 flex justify-between text-white"><span>Avg. Challenges per User</span><span>0.0</span></div>
              <div className="mb-2 flex justify-between text-white"><span>Active Users</span><span>0 / 24</span></div>
            </div>
          </div>
        )}
              ...
              <table className="min-w-full text-left">
                <thead>
                  <tr className="border-b border-white/20">
                    ...
                    <th className="py-2 px-4 text-white font-semibold">Joined Date</th>
                    <th className="py-2 px-4 text-white font-semibold">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {mockUsers.map((user, i) => (
                    <tr key={i} className="border-b border-white/10 hover:bg-white/10">
                      ...
                      <td className="py-2 px-4 text-white/80">{user.joined}</td>
              ...
              <table className="min-w-full text-left">
                <thead>
                  <tr className="border-b border-white/20">
                    ...
                    <th className="py-2 px-4 text-white font-semibold">Name</th>
                    <th className="py-2 px-4 text-white font-semibold">Email</th>
                    <th className="py-2 px-4 text-white font-semibold">Subject</th>
                    <th className="py-2 px-4 text-white font-semibold">Message</th>
                    <th className="py-2 px-4 text-white font-semibold">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {mockMessages.map((msg, i) => (
                    <tr key={i} className="border-b border-white/10 hover:bg-white/10">
                      <td className="py-2 px-4 text-white/80">{msg.date}</td>
                      ...
                      <td className="py-2 px-4 text-white/80">{msg.message}</td>
        ...
        {tab === 'organizations' && (
          <div className="glassmorphism rounded-2xl p-6 mb-12 text-center">
            ...
            <p className="text-lg text-white/80">Organization management coming soon.</p>
          </div>
        )}
      </div>
    </Layout>
